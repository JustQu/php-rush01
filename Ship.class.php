...
class Ship
{
	...
	private function check_op($opponent, $y, $x)
	{
		foreach($opponent as $op) {
			if ($y >= $op->getY() && $y < ($op->getY() + $op->getSizeY()) && $x >= $op->getX() && $x < ($op->getX() + $op->getSizeX()))
				return (array($op->getX(), $op->getY()));
		}
		return (false);
	}
	public function shoot($board, $opponent, $damage)
	{
		$y = $this->_y;
		if ($this->direction == 1)
		{
			for ($i = 0; $i <= $this->_weapon->_Lrange[1] && $y < 100; $i++ && $y++) {
				if ($board[$y][ $this->_x])
					return (false);
				elseif (($pos = check_op($opponent, $y, $this->_x)))
					return ($pos);
			}
		}
		else
